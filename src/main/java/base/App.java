/*
 *  UCF COP3330 Summer 2021 Assignment 1 Solutions
 *  Copyright 2021 Liam Pido
 */

/*
Exercise 18 - Temperature Converter

You’ll often need to determine which part of a program is run based on user input or other events.

Create a program that converts temperatures from Fahrenheit to Celsius or from Celsius to Fahrenheit. Prompt for the starting temperature.
The program should prompt for the type of conversion and then perform the conversion.

The formulas are

C = (F − 32) × 5 / 9

and

F = (C × 9 / 5) + 32

Example Output

Press C to convert from Fahrenheit to Celsius.
Press F to convert from Celsius to Fahrenheit.
Your choice: C
Please enter the temperature in Fahrenheit: 32
The temperature in Celsius is 0.

Constraints

Ensure that you allow upper or lowercase values for C and F.
Use as few output statements as possible and avoid repeating output strings.

Challenges

Revise the program to ensure that inputs are entered as numeric values. Don’t allow the user to proceed if the value entered is not numeric.
Break the program into functions that perform the computations.
Implement this program as a GUI program that automatically updates the values when any value changes.
Modify the program so it also supports the Kelvin scale.
 */

package base;

import java.util.Scanner;

public class App {
    Scanner in = new Scanner(System.in);
    public static void main(String[] args) {
        App myApp = new App();

        String input = myApp.readInput();
        double conversion = myApp.conversion(input);
        myApp.output(input, conversion);
    }

    public void output(String input, double conversion) {
        if(input.toUpperCase().equals("C")) {
            System.out.println("The temperature in Celsius is " + String.format("%.1f", conversion) + ".");
        } else if(input.toUpperCase().equals("F")) {
            System.out.println("The temperature in Fahrenheit is " + String.format("%.1f", conversion) + ".");
        }
    }

    public double conversion(String input) {
        double conversion = 0;

        if(input.toUpperCase().equals("C")) {
            System.out.print("Please enter the temperature in Fahrenheit: ");
            String temp = in.nextLine();
            int temperature = Integer.parseInt(temp);
            conversion = (temperature - 32.0) * (5.0 / 9.0);
        } else if(input.toUpperCase().equals("F")) {
            System.out.print("Please enter the temperature in Celsius: ");
            String temp = in.nextLine();
            int temperature = Integer.parseInt(temp);
            conversion = ((temperature * 9.0) / 5.0) + 32.0;
        }

        return conversion;
    }

    public String readInput() {
        System.out.println("Press C to convert from Fahrenheit to Celsius.");
        System.out.println("Press F to convert from Celsius to Fahrenheit.");
        System.out.print("Your choice: ");
        String input = in.nextLine();
        return input;
    }
}
